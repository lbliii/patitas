    ...
    def test_markdown_uses_correct_plugin_names(self) -> None:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Compare):
                # Look for "x in self._plugins" patterns
                if (len(node.comparators) == 1 and
                    isinstance(node.comparators[0], ast.Attribute) and
                    node.comparators[0].attr == "_plugins"):
                    # Get the left side (should be a string)
                    if isinstance(node.left, ast.Constant) and isinstance(node.left.value, str):
                        plugin_checks.append(node.left.value)
    ...
    def test_plugin_name_matches_module_name(self) -> None:
        ...
        for name in BUILTIN_PLUGINS:
            try:
                ...
                mod = importlib.import_module(f"patitas.plugins.{name}")
                # Module exists, which is good
            except ModuleNotFoundError:
                # Some plugins might use different module names (e.g., plurals)
                # Document these as warnings, not failures
                pass
