...
| Autolinks (bare URL/email) | `Markdown(plugins=["autolinks"])` | Behavioral tests in `tests/test_autolinks.py`. Recognizes bare `http(s)://`, `www.` (href prefixed with `http://`), and bare `user@host` emails (`mailto:`) following the GFM left-boundary and trailing-punctuation rules. URL/`www.` bodies run until whitespace or `<`, so `& ~ $ _` etc. inside a URL (query separators, `~user` paths, `Foo_(bar)` slugs) are kept in the link. Known limitations: a backtick ends a URL body (code spans keep higher precedence), and an email local part split by an inline-special delimiter (e.g. `a_b@example.com`) links only from the delimiter onward. CommonMark angle-bracket autolinks `<https://...>` work without any plugin. |
