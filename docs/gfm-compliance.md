...
Patitas passes **654 of 672** GFM 0.29 spec examples (**97.3%**).
...
count never drops below the recorded baseline. The 18 known-failing examples are
...
### Why the remaining 18 fail

| Count | Category | Reason |
|---|---|---|
| 4 | Autolinks | GFM extended autolinks are implemented, but 4 fixture examples diverge: 616/619/620 are core CommonMark "Autolinks" examples that assume the extension is *off* (with it enabled, as GFM intends, Patitas links the bare URL/email); 628 links its http(s) sub-cases but not `ftp://`. Example 631 (email local part containing `_`) now passes: the inline tokenizer resolves the bare email before emphasis. |
...
| Autolinks (bare URL/email/`www.`) | `Markdown(plugins=["autolinks"])` | GFM extended autolinks implemented: bare `http(s)://`, `www.` (href prefixed `http://`), and `user@host` emails link; URL bodies keep `& ~ $ _` etc.; trailing-punctuation, paren-balance, and entity trimming follow the GFM rules. Edge cases not covered: `ftp://`. Email local parts containing `_` (e.g. `a_b@example.com`) link in full. CommonMark angle-bracket autolinks `<https://...>` work without any plugin. |
